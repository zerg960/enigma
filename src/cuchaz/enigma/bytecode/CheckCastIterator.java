...
public class CheckCastIterator implements Iterator<CheckCast> {
	...
	while (m_iter.hasNext()) {
		...
		// get the type of this op code (next two bytes are a classinfo
		// index)
	...
	case Opcode.INVOKESPECIAL: {
	    ...
	    return new MethodEntry(new ClassEntry(Descriptor.toJvmName(m_constants.getMethodrefClassName(index))),
		    m_constants.getMethodrefName(index), m_constants.getMethodrefType(index));
	}

	case Opcode.INVOKEINTERFACE: {
	    int index = m_iter.s16bitAt(pos + 1);
	    return new MethodEntry(new ClassEntry(Descriptor.toJvmName(m_constants
		    .getInterfaceMethodrefClassName(index))), m_constants.getInterfaceMethodrefName(index),
