...
public class BridgeFixer {
	...
	for (CtMethod method : c.getDeclaredMethods()) {
	    ...
	    MethodEntry methodEntry = new MethodEntry(new ClassEntry(Descriptor.toJvmName(c.getName())),
	...
	for (int i = 1; i < constants.getSize(); i++) {
		...
		if (behaviorEntry instanceof MethodEntry) {
			...
			editor.changeMemberrefNameAndType(i, bridgeMethodEntry.getName(),
				bridgeMethodEntry.getSignature());
		    }
		}
	    }
		break;
