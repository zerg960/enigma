...
public class Gui {
	...
	JSplitPane callPanel = new JSplitPane(JSplitPane.VERTICAL_SPLIT, true, new JScrollPane(m_callsTree),
	...
	final Timer timer = new Timer(200, new ActionListener() {
		...
		if (m_counter % 2 == 0) {
		    try {
			m_highlight = m_editor.getHighlighter().addHighlight(token.start, token.end,
				m_selectionHighlightPainter);
		    } catch (BadLocationException ex) {
			// don't care
		    }
	...
	panel.add(GuiTricks.unboldLabel(new JLabel(value, SwingConstants.LEFT)));
    }
	...
	JPanel panel = (JPanel) m_infoPanel.getComponent(0);
	panel.remove(panel.getComponentCount() - 1);
	panel.add(GuiTricks.unboldLabel(new JLabel(m_reference.getNamableName(), SwingConstants.LEFT)));
	...
	if (m_reference.entry instanceof ClassEntry) {
	    ...
	    // TODO: get a list of all the constructors and find calls to all of
	    // them
	    BehaviorReferenceTreeNode node = m_controller.getMethodReferences(new ConstructorEntry(
		    (ClassEntry) m_reference.entry, "()V"));
	...
	if (!m_controller.isDirty()) {
	    ...
	    int response = JOptionPane.showOptionDialog(m_frame,
		    "Your mappings have not been saved yet. Do you want to save?", "Save your changes?",
		    JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE, null, options, options[2]);
	    switch (response) {
	    case JOptionPane.YES_OPTION: // save and exit
		if (m_mappingsFileChooser.getSelectedFile() != null
			|| m_mappingsFileChooser.showSaveDialog(m_frame) == JFileChooser.APPROVE_OPTION) {
		    try {
			m_controller.saveMappings(m_mappingsFileChooser.getSelectedFile());
			m_frame.dispose();
		    } catch (IOException ex) {
