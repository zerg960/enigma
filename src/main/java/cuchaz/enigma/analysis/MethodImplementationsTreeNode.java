...
public class MethodImplementationsTreeNode extends DefaultMutableTreeNode {
	...
	for (String implementingClassName : index.getImplementingClasses(m_entry.getClassName())) {
	    MethodEntry methodEntry = new MethodEntry(new ClassEntry(implementingClassName), m_entry.getName(),
	...
	for (int i = 0; i < node.getChildCount(); i++) {
	    MethodImplementationsTreeNode foundNode = findNode((MethodImplementationsTreeNode) node.getChildAt(i),
		    entry);
	    if (foundNode != null) {
		return foundNode;
	    }
